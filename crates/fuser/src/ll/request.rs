...
mod op {
    ...
    impl<'a> Rename<'a> {
        ...
        pub(crate) fn flags(&self) -> RenameFlags {
            self.flags
        }
    }
    ...
    pub(crate) fn parse<'a>(
        ...
        Some(match opcode {
            ...
            fuse_opcode::FUSE_RENAME => {
                ...
                #[cfg(target_os = "macos")]
                {
                    // macFUSE 4.x sent flags+padding unconditionally after
                    // newdir; macFUSE 5.x only sends that tail when rename
                    // capabilities are negotiated. Detect the tail at runtime
                    // so one binary works with both protocols.
                    if data.peek_byte(0).is_some_and(|byte| byte < 32) {
                        ...
                        if !data.skip_bytes(8) {
                            return None;
                        }
                        flags = RenameFlags::from_bits_retain(raw_flags);
                    }
                }
...
mod tests {
    ...
    fn fill_rename_header(buf: &mut [u8], len: usize) {
        write_u32(buf, 0, len as u32);
        write_u32(buf, 4, crate::ll::fuse_abi::fuse_opcode::FUSE_RENAME as u32);
        write_u64(buf, 8, 0xdead_beef_baad_f00d);
        write_u64(buf, 16, 0x1122_3344_5566_7788);
        write_u32(buf, 24, 1000);
        write_u32(buf, 28, 1000);
        write_u32(buf, 32, 42);
        write_u32(buf, 36, 0);
    }

    #[test]
    fn rename_short_format_without_flags() {
        const LEN: usize = 56;
        let mut data = AlignedData([0u8; LEN]);
        fill_rename_header(&mut data.0, LEN);
        write_u64(&mut data.0, size_of::<fuse_in_header>(), 1);
        data.0[48..56].copy_from_slice(b"old\0new\0");

        let req = AnyRequest::try_from(&data.0[..]).unwrap();
        match req.operation().unwrap() {
            Operation::Rename(x) => {
                assert_eq!(x.src().name.as_os_str(), OsStr::new("old"));
                assert_eq!(x.dest().dir, INodeNo(1));
                assert_eq!(x.dest().name.as_os_str(), OsStr::new("new"));
                assert_eq!(x.flags().bits(), 0);
            }
            _ => panic!("Unexpected request operation"),
        }
    }
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn rename_extended_macos_format_with_flags() {
        const LEN: usize = 64;
        let mut data = AlignedData([0u8; LEN]);
        fill_rename_header(&mut data.0, LEN);
        write_u64(&mut data.0, size_of::<fuse_in_header>(), 1);
        write_u32(&mut data.0, 48, 3);
        write_u32(&mut data.0, 52, 0);
        data.0[56..64].copy_from_slice(b"old\0new\0");
