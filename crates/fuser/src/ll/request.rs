    ...
    pub(crate) fn parse<'a>(
        ...
        Some(match opcode {
            ...
            fuse_opcode::FUSE_RENAME => {
                ...
                // macFUSE supports extended rename operations via flags in the rename request.
                // When capabilities are negotiated during FUSE_INIT, macFUSE sends an extended
                // 16-byte fuse_rename_in: newdir (8 bytes), flags (4 bytes), padding (4 bytes).
                //
                // macFUSE 4.x bug: sent extended format unconditionally regardless of capabilities.
                // macFUSE 5.x fixed: only sends extended format when capabilities ARE granted.
                //
                // Detection heuristic: filenames cannot start with null bytes or ASCII control
                // characters (< 32). If first byte after newdir is < 32, we have extended format.
                #[cfg(target_os = "macos")]
                let flags = {
                    ...
                    if first_byte.is_some_and(|b| b < 32) {
                        ...
                        data.skip_bytes(8); // Skip flags (4) + padding (4)
                        flags
                    } else {
                        0
                    }
