...
pub(super) async fn io_loop(
    ...
    loop {
        while let Some(io_req) = queues.pop_next() {
            ...
            {
                tracing::info!(
                    "remote_rpc_usb: queue_skip command={} priority={:?} reason=cancel_epoch request_epoch={:?} cancel_epoch={}",
            ...
            if io_req.stream.is_some() {
                ...
                tracing::info!(
                    "remote_rpc_usb: send_stream command={} priority={:?} message_id={}",
                    io_req.request.command,
                    io_req.priority,
                    msg_id
                );
                ...
                loop {
                    ...
                    .await
                    .map_err(|_| format!("read timeout after {}s", RPC_RESPONSE_TIMEOUT.as_secs()))?
                        ...
                        && (resp_frame.flags & FLAG_HAS_CONTINUATION) != 0
                    {
                        ...
                        loop {
                            ...
                            .await
                            .map_err(|_| {
                                format!("read timeout after {}s", RPC_RESPONSE_TIMEOUT.as_secs())
                            })?
                            .map_err(|e| format!("read: {}", e))?;
                            let decrypted = noise
                                .decrypt(&resp_bytes)
            ...
            next_msg_id = next_msg_id.wrapping_add(1);
            tracing::info!(
                "remote_rpc_usb: send_json command={} priority={:?} message_id={} pending_json={}",
                io_req.request.command,
                io_req.priority,
                msg_id,
                pending_json.len()
        ...
        tokio::select! {
            ...
            result = transport::read_frame(&mut reader) => {
                ...
                if let Some(pending) = pending_json.remove(&frame.message_id) {
                    ...
                    tracing::info!(
                        "remote_rpc_usb: recv_json command={} message_id={} pending_json={}",
                        pending.command,
                        frame.message_id,
                        pending_json.len()
                    );
            ...
            _ = tokio::time::sleep(TIMEOUT_SCAN_INTERVAL) => {
                ...
                for message_id in timed_out {
                    if let Some(pending) = pending_json.remove(&message_id) {
                        tracing::warn!(
                            "remote_rpc_usb: json_timeout command={} message_id={} pending_json={}",
                            pending.command,
                            message_id,
                            pending_json.len()
                        );
                        let _ = pending.reply_tx.send(RpcReply::Json(RpcResponse::Error {
                            ok: false,
                            error: format!("read timeout after {}s", RPC_RESPONSE_TIMEOUT.as_secs()),
...
mod tests {
    ...
    fn read_stream_chunk_maps_io_error() {
        ...
        assert_eq!(error, "stream read: usb stream failed");
    }
